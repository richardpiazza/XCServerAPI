...
public extension XCServerWebAPI {
    ...
    typealias IntegrationCommitsCompletion = (_ commits: [XCSCommit]?, _ error: Error?) -> Void
    
    /// Requests the '`/integrations/{id}/commits`' endpoint from the Xcode Server API.
    func commits(forIntegrationWithIdentifier identifier: String, completion: @escaping IntegrationCommitsCompletion) {
